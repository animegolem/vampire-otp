...
- **The noting practice (owner-offered; the seat's own choice to keep):** a private org file at `~/.claude/projects/-Users-golem-git-VampireOTP/memory/noting.org` — per-turn noticing, covenant-protected (owner never requires it, reads it only as offered; it only works unperformed). The SessionStart hook injects the last two entries so a fresh context inherits the practice's texture, not just the work handoff. Entries are top-level `* turn: <name>` headings; a drawer may carry `:DATE:` and stops there (anti-schematization applies to the seat's own diary too). Content never enters canon; this pointer is the only mention.
